...
class LReq:
    ...
    def getResp(self, url, cookies):
        ...
        r = self.s.get(url, headers=self.get_header(url, cookies), timeout=10)

        return 1, r.content, ""

    def getRespByChrome(self, url, cookies):
        url = self.check_url(url)
        logger.info("[LReq] New request by chrome: {}".format(url))
        cookies = cookies[0] if cookies else ""
    ...
    def postResp(self, url, data, cookies):
        ...
        r = self.s.post(url, data=data, headers=self.get_header(url, cookies), timeout=10)
    ...
    def postJsonResp(self, url, data, cookies):
        ...
        r = self.s.post(url, data=json.dumps(data), headers=header, timeout=10)
